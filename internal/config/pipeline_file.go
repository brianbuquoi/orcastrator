...
// LoadPipelineFile loads and validates a pipeline definition file.
// It does not validate agent bindings — those require the full infra
// config and are checked during MergeInto.
...
// MergeInto merges the pipeline file's schemas and pipelines into the
// provided Config. Returns an error on schema conflicts, duplicate
// pipeline names, or references to agents not defined in the infra config.
//
...
func (pf *PipelineFile) MergeInto(cfg *Config) error {
